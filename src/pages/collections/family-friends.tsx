...
export default function FamilyFriends(): JSX.Element {
  const blogData = usePluginData('docusaurus-plugin-content-blog', 'default') as any;
  const allPosts = blogData?.blogPosts || [];

  // Filter posts by tag
  const filteredPosts = allPosts.filter((post: any) =>
    post.metadata.tags.some((tag: any) => tag.label === 'family-friends')
  );

  return (
    <Layout
      title="Family & Friends"
      description="Life, relationships, and the people who matter most.">
      <main className="container margin-vert--lg">
        <div className="row">
          <div className="col col--8 col--offset-2">
            <h1>Family & Friends</h1>
            <p className="margin-bottom--lg">
              Life, relationships, and the people who matter most.
            </p>
